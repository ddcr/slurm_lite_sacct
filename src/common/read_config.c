...
s_p_options_t slurm_conf_options[] = {
	...
	/*  NLK changes */
	{"SlurmEventHandler", S_P_STRING},
	{"SlurmEventHandlerLogfile", S_P_STRING},
	{"SlurmEventHandlerPollInterval", S_P_UINT16},
	/* NLK changes */
	...
	{"SlurmKillOnBadExit", S_P_BOOLEAN},
	...
	/* The following keywords are ignored by this parser and handled
	   by the slurm_setup script */
	{"SlurmGroup", S_P_IGNORE},
	{"SlurmHome", S_P_IGNORE},
	{"SlurmUID", S_P_IGNORE},
	{"SlurmGID", S_P_IGNORE},
...
free_slurm_conf (slurm_ctl_conf_t *ctl_conf_ptr, bool purge_node_hash)
{
	...
	/* nlk Changes */
	xfree (ctl_conf_ptr->slurm_event_handler);
	xfree (ctl_conf_ptr->event_handler_logfile);
	/* nlk Changes */
...
_validate_and_set_defaults(slurm_ctl_conf_t *conf, s_p_hashtbl_t *hashtbl)
{
	...
	if (!s_p_get_boolean((bool *)&conf->slurm_kill_on_bad_exit, "SlurmKillOnBadExit", hashtbl))
		conf->slurm_kill_on_bad_exit = DEFAULT_SLURM_KILL_ON_BAD_EXIT;
	...
	/* nlk Changes */
	s_p_get_string(&conf->slurm_event_handler, "SlurmEventHandler", hashtbl);
	s_p_get_string(&conf->event_handler_logfile, "SlurmEventHandlerLogfile", hashtbl);
	if (!s_p_get_uint16(&conf->event_handler_poll_interval, "SlurmEventHandlerPollInterval", hashtbl))
			conf->event_handler_poll_interval = DEFAULT_EVENT_HANDLER_POLL_TIME;
