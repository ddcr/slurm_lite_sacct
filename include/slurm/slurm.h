...
#define SLURM_API_VERSION 0x140000
#endif
...
typedef struct slurm_ctl_conf {
	...
	uint16_t slurm_kill_on_bad_exit; /* If set, the job will be */
				/* terminated immediately when one of */
				/* the processes is aborted or crashed */ 
	...
	/* char *salloc_default_command;  default salloc command */
	/*NLK event handler Changes */
	char *slurm_event_handler; /* pathname of event handler */
	uint16_t event_handler_poll_interval; /* poll interval of event handler */
	char *event_handler_logfile; /* event handler logfile */
	/*NLK event handler Changes */
} slurm_ctl_conf_t;
