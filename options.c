...
int print_job_cond(acct_job_cond_t *job_cond) {
	ListIterator itr = NULL;
	char *extra = NULL; // @author Domingos Rodrigues
	char *object = NULL;
	int set = 0;

	debug4("Entrando em print_job_cond()");

	if(job_cond->acct_list && list_count(job_cond->acct_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->acct_list = [");
	...
	if(job_cond->associd_list && list_count(job_cond->associd_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->associd_list = [");
	...
	if(job_cond->cluster_list && list_count(job_cond->cluster_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->cluster_list = [");
	...
	if(job_cond->groupid_list && list_count(job_cond->groupid_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->groupid_list = [");
	...
	if(job_cond->partition_list && list_count(job_cond->partition_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->partition_list = [");
	...
	if(job_cond->resv_list && list_count(job_cond->resv_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->resv_list = [");
	...
	if(job_cond->resvid_list && list_count(job_cond->resvid_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->resvid_list = [");
	...
	if(job_cond->state_list && list_count(job_cond->state_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->state_list = [");
	...
	if(job_cond->usage_start) {
		...
		debug4("job_cond->usage_start = %s (%u s)", start_char, job_cond->usage_start);
		xfree(start_char);
	} else {
	...
	if(job_cond->usage_end) {
		...
		debug4("job_cond->usage_end = %s (%u s)", end_char, job_cond->usage_end);
		xfree(end_char);
	} else {
	...
	if(job_cond->userid_list && list_count(job_cond->userid_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->userid_list = [");
		...
		debug4("job_cond->userid_list is empty");
	}

	if(job_cond->wckey_list && list_count(job_cond->wckey_list)) {
		set = 0;
		xstrfmtcat(extra, "job_cond->wckey_list = [");
	...
	debug4("job_cond->without_usage_truncation = %u", job_cond->without_usage_truncation);

	debug4("Saindo de print_job_cond()");
	return 1;
}
...
void parse_command_line(int argc, char **argv)
{
	...
	if(params.opt_completion) {
		...
		// acct_type="accounting_storage/mysql"; /* DDCR note */
