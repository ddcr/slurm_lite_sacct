...
// Either #define _GNU_SOURCE or just put the prototype here (safer?)
...
#define _PATH_GROUP		"etc/slurm/group.veredas"
#define _PATH_PASSWD	"etc/slurm/passwd.veredas"
...
int compat_getpwuid_r(uid_t key,
...
struct passwd *compat_getpwnam(const char *name)
{
	...
	static struct passwd resultbuf;
	struct passwd *result;
	compat_getpwnam_r(name, &resultbuf, buffer, sizeof(buffer), &result);
	return result;
}

int compat_getpwnam_r(const char *__restrict key,
					  struct passwd *__restrict resultbuf,
					  char *__restrict buffer, size_t buflen,
					  struct passwd **__restrict result)
{
...
struct group *compat_getgrgid(gid_t gid)
{
	...
	compat_getgrgid_r(gid, &resultbuf, buffer, sizeof(buffer), &result);
	return result;
}

int compat_getgrgid_r(gid_t key,
...
struct group *compat_getgrnam(const char *name)
{
    ...
    compat_getgrnam_r(name, &resultbuf, buffer, sizeof(buffer), &result);
    return result;
}

int compat_getgrnam_r(const char *__restrict key,
					  struct group *__restrict resultbuf,
					  char *__restrict buffer, size_t buflen,
					  struct group **__restrict result)
{
